...
def test_missing_env_vars():
    ...
    with patch.dict("os.environ", {}, clear=True):
        with pytest.raises(EnvironmentError) as exc_info:
            GoogleProvider()
        assert "Missing required Google environment variable" in str(exc_info.value)


def test_genai_interface():
    ...
    def test_text_response():
        ...
        interface = GoogleProvider(api_key="fake-key")
        mock_response = MagicMock()
...
def test_convert_openai_to_genai():
    """Test the message conversion from OpenAI format to GenAI format."""
...
def test_role_conversions():
    ...
    # System and user messages should both be converted to "user" role in GenAI
    assert len(result) == 3
