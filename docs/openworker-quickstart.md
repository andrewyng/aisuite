# OpenWorker quickstart

[OpenWorker] is a desktop AI agent that can not only chat, but also do deep research and carry out tasks for you on your computer. It can read files (with permission) to gain context, read/send messages (slack, email, etc.), and create real deliverables like PDF reports, documents, spreadsheets. It also supports scheduled automations, such as providing you a daily news summary.

## 1. Install

| Platform | Download |
|---|---|
| macOS 13+ (Apple Silicon, M1 or later) | [OpenWorker-macos-arm64.dmg](https://github.com/andrewyng/aisuite/releases/latest/download/OpenWorker-macos-arm64.dmg) |
| Windows 10/11 (x64) | [OpenWorker-windows-setup.exe](https://github.com/andrewyng/aisuite/releases/latest/download/OpenWorker-windows-setup.exe) |

NOTE: On Windows, SmartScreen may warn on first run: choose **More info → Run anyway** (the build isn't Authenticode-signed yet).

## 2. Connect a model

Pick a provider and paste your API key — OpenAI, Anthropic (Claude), or Google (Gemini) — or select **Ollama** to run fully local with no key at all. Keys are stored on your machine and sent only to the provider you chose; there is no OpenWorker server.
...
Ask for recurring work and OpenWorker schedules it:
...
OpenWorker speaks the Model Context Protocol. Add servers in **Manage → Integrations** using the same `mcpServers` JSON format as Claude Desktop and Cursor — stdio or HTTP, with per-tool approval controls.

## Privacy

Your API keys, your conversations, and your files stay on your machine. OpenWorker has no backend — model calls go directly from your computer to the provider you configured.
